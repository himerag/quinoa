...
Ntrainings=20;
NumEpoc=6;
ItxEp=148; %Iterations per epoch
...
TimeElap = zeros(1,Ntrainings);
for NTipo=1:length(Tipo)
    display(strcat('En evaluacion = ',Tipo{NTipo}))
    % concadenating folder names
    carpeta =strcat(cd,'\DataSetQuinoa227x227\Imagenes_',num2str(Tamano),'_',Tipo{NTipo});
    % loading images datasets
    imds = imageDatastore(carpeta, 'IncludeSubfolders',true, 'LabelSource','foldernames');
    % loading the network
    net = alexnet;
    ...
    for i=1:Ntrainings
        ...
        tic
        % network training
        [netTransfer, info] = trainNetwork(imdsTrain,layers,options);
        TimeElap(1,i)=toc;
